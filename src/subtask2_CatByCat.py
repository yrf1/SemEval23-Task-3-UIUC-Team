...
python scorers/scorer-subtask-2.py -p baselines/our-train-output-subtask3-en.txt -g data/en/train-labels-subtask-3.txt --techniques_file_path scorers/techniques_subtask3.txt
python scorers/scorer-subtask-2.py -p baselines/our-train-output-subtask2-en2.txt -g data/en/train-labels-subtask-22.txt --frame_file_path frames_subtask2.txt

plan A: pretrain on all class, finetune 11 individual classifiers
plan B: backward MT, use definitions
plan C: 

Leaderboard top	0.78481	0.55745
Baseline 0.60545	0.39752
Ours, currently (official): 0.70667	0.44813
Ours, currently (local): micro-F1=0.63162	macro-F1=0.60259
"""
import os
import torch
import random
...
LABELS_OF_INTEREST = [k for k,v in label_count.items() if v>20]
print(label_count)
print(len(LABELS_OF_INTEREST))
#quit()
LABELS_DEF = pd.read_csv("resources/task2_def.csv",header=None)
...
class MyDataset(Dataset):
    def __init__(self, mode="train", cross_val_split_idx=-1, all_labels=None):
        ...
        for lang_dir in os.listdir(data_dir):
            ...
            if lang_dir==lang and mode=="train" and cross_val:
                pass #del self.data_lang[cross_val_split_idx::5]
        if mode=="val":
            self.data_all = self.data_all[cross_val_split_idx::5]
        if mode=="train":
            del self.data_all[cross_val_split_idx::5]
        self.mode = mode
        self.tokenizer = AutoTokenizer.from_pretrained("MoritzLaurer/DeBERTa-v3-base-mnli-fever-docnli-ling-2c") #"bigscience/T0pp" if "T5" in model_name else model_name)
        #self.tokenizer = self.tokenizer.add_tokens(['<S>','<Q>','<A>'])
        print("A", len([x for x in self.data_lang if x[-2] in x[-1]]))
        print("B", len([x for x in self.data_lang if x[-2] not in x[-1]]))
        #quit()
    ...
    def __getitem__(self, idx):
        ...
        #print(txt1)
        #print(txt2)
        #print(propaganda, this_all_labels)
        #print("~~~~~~")
        #quit()
        #txt2 = txt2.replace("_"," ")
        if model_name == "facebook/bart-large-mnli":
            txt2_exp = txt2.replace("_"," ")
            if use_def:
...
for cross_val_split_idx in range(5):
    ...
    for (mode, tot_eps, dataloader) in [\
            ("train",5,train_dataloader), ("val",1 if (cross_val or \
        ...
        for ep in range(tot_eps):
            ...
            for idx, (fname, segID, x, prop_idx, y) in enumerate(dataloader):
                ...
                if mode in ["val","dev"]:
                    ...
                    segID = segID.item()
                if mode in ["val"]:
                    val_pred_tracker.append(pred_y)
                    val_gt_tracker.append(y.item())
    ...
    #    break
        ...
        if mode=="val":
            ...
            train_results_tracker_df = pd.DataFrame(data)
            train_results_tracker_df.to_csv("baselines/our-train-output-subtask2-"+lang+("_def" if use_def else "")+".txt", \
